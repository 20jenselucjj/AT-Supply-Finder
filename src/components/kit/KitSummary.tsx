  ...
  return (
    <Card className="p-4 xs:p-6 mt-6" aria-labelledby="kit-summary-heading">
      ...
      <div className="space-y-3">
        <h3 className="font-medium text-sm xs:text-base">Best prices by vendor</h3>
        {vendorTotals.map(({ vendor, total, url }) => {
          const isBest = bestVendor && bestVendor.vendor === vendor;
          return (
            <div
              key={vendor}
              className="flex flex-col xs:flex-row justify-between xs:items-center gap-2 xs:gap-3 p-3 rounded-md border"
            >
              <div className="flex items-center gap-2">
                <span className="text-sm xs:text-base">{vendor}</span>
                {isBest && <Badge className="text-xs">Best price</Badge>}
              </div>
              <div className="flex items-center justify-between xs:justify-end gap-2">
                <span className="font-medium text-sm xs:text-base">{formatCurrency(total)}</span>
