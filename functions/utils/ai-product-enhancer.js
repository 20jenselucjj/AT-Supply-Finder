...
// Function to enhance product data using OpenRouter AI with fallback to Gemini
export async function enhanceProductWithAI(productData, openRouterApiKey, geminiApiKey) {
  // First attempt with OpenRouter
  if (openRouterApiKey) {
    try {
      const enhancedData = await enhanceWithOpenRouter(productData, openRouterApiKey);
      if (enhancedData) {
        console.log('Successfully enhanced product with OpenRouter');
        return enhancedData;
      }
    } catch (error) {
      console.warn('OpenRouter enhancement failed, trying Gemini fallback:', error.message);
    }
  }

  // Fallback to Gemini if OpenRouter fails or no OpenRouter key
  if (geminiApiKey) {
    try {
      const enhancedData = await enhanceWithGemini(productData, geminiApiKey);
      if (enhancedData) {
        console.log('Successfully enhanced product with Gemini fallback');
        return enhancedData;
      }
    } catch (error) {
  ...
  // If both AI services fail, return original data
  console.warn('Both OpenRouter and Gemini failed, returning original product data');
  return productData;
}

// OpenRouter enhancement function
async function enhanceWithOpenRouter(productData, openRouterApiKey) {
  try {
...
`;
    ...
    console.error('Error enhancing product with OpenRouter:', error.message);
    throw error; // Re-throw to trigger fallback
  }
}

// Gemini enhancement function
async function enhanceWithGemini(productData, geminiApiKey) {
  try {
...
`;
    ...
    console.error('Error enhancing product with Gemini:', error.message);
